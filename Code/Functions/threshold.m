...
if (get(Exec, 'value') == 0) && (S.plotmanual.Value ~= 1);
    S.dspikeven.Value=0; S.dphaspace.Value=0;
    cla(S.ax(6)); axes(S.ax(6)); S.ax(6).NextPlot = 'replaceall';
    cla(S.ax(7)); axes(S.ax(7)); S.ax(7).NextPlot = 'replaceall';
    return; 
end
...
if S.ppeak.Value == 1 % Positive peak detection
    Y1 = plot(ax5, [tim5 tim6], 5 * [Sigma, Sigma], '--r', 'linewidth', 2); 
    Leg = {['Thr^+:', num2str(5 * Sigma)]};
